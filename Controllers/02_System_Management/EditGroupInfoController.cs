...
namespace rc_interface_API.Controllers._02_System_Management
{
    ...
    [ApiController]
    public class EditGroupInfoController : ControllerBase
    {
        [HttpPost]
        public object FormAll(Info_EditGroupInfoModel Data)
        {
            ...
            if (InputIsok == "N")
            {
                APCommonFun.Error("[EditGroupInfoController]90-" + ReturnErr);
                return new
                {
                    Result = "R",
                    Message = ReturnErr,
            ...
            string sql2 = "select * FROM  groups where GRPID='" + group_code + "' ";
            ...
            try
            {
                ...
                if (dt2.Rows.Count > 0)
                {
                    sql = "update groups set GRPNAME='" + group_name
                         + "' , GRPdes='" + note  + "' where GRPID='" + group_code + "' ";
            ...
            {
                APCommonFun.Error("[EditGroupInfoController]99：" + ex.ToString());
                APCommonFun.Error("[EditGroupInfoController]99：" + sql);
