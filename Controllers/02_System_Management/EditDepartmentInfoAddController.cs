...
namespace rc_interface_API.Controllers._02_System_Management
{
    ...
    [ApiController]
    public class EditDepartmentInfoAddController : ControllerBase
    {
        [HttpPost]
        public object FormAll(Info_EditDepartmentInfoModel Data)
        {
            ...
            //第一步 : 先判斷有沒有必填未填寫，
            if (department_id == "" || department_name == "" ||  is_receive_mail == "") //必填                       
            {
                if (department_id == "")
                {
                    InputIsok = "N";
                    ReturnErr = "執行動作錯誤-department_id 為必填欄位";
                }
            ...
            if (InputIsok == "N")
            {
                APCommonFun.Error("[EditDepartmentInfoAddController]90-" + ReturnErr);
                return new
                {
                    Result = "R",
                    Message = ReturnErr,
            ...
            string sql2 = "select * FROM  departments where Upper(deptid)='" + department_id + "' ";
            ...
            try
            {
                ...
                if (dt2.Rows.Count == 0)
                {
                    sql = "Insert Into departments(deptid, deptname, deptdes, canbecontactedbycust) Values ('"
                            + department_id + "', '" + department_name + "', '" + description + "', '" + is_receive_mail + "' )";
            ...
            {
                APCommonFun.Error("[EditDepartmentInfoAddController]99：" + ex.ToString());
                APCommonFun.Error("[EditDepartmentInfoAddController]99：" + sql);
