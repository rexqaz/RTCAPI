...
namespace rc_interface_API.Controllers._02_System_Management
{
    ...
    [ApiController]
    public class EditAccountInfoController : ControllerBase
    {
        [HttpPost]
        public object FormAll(Info_EditAccountInfoModel Data)
        {
            ...
            if (InputIsok == "N")
            {
                APCommonFun.Error("[EditAccountInfoController]90-" + ReturnErr);
                return new
                {
                    Result = "R",
                    Message = ReturnErr,
            ...
            try
            {
                ...
                if (dt2.Rows.Count > 0)
                {
                    sql = "update ACCOUNTS set GRPID='" + group + "', ACCPWD='" + Convert.ToBase64String(Encoding.UTF8.GetBytes(new_password)) 
                         + "' , ISEMPLOYEE='" + is_employee + "',  EMPID='" + employee_id + "', ACCDES='" + note + "' where ACCID='" + account + "' ";
            ...
            {
                APCommonFun.Error("[EditAccountInfoController]99：" + ex.ToString());
                APCommonFun.Error("[EditAccountInfoController]99：" + sql);
